...
describe("composer hint UI", () => {
  ...
  it("renders unavailable score and deterministic hints after debounced local composer typing", async () => {
    ...
    const hint = panel?.querySelector<HTMLElement>(".scoreboar-composer-panel__hint")
    expect(hint?.title).toBe("Open with a clear question, claim, or tension.")
    expect(hint?.getAttribute("aria-label")).toBe("Open with a clear question, claim, or tension.")
    expect(panel?.getAttribute("aria-label")).toContain("Open with a clear question, claim, or tension.")
